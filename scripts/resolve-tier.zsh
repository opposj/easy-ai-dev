typeset -A LANG_MAP=(c '' cpp '' csharp 'dotnet' ruby '' fortran '' perl '' python 'uv python' js 'bun node' tsc 'bun node' deno 'deno' go 'go' rust 'rustup' sql 'duckdb' php 'php' lua 'lua luarocks' elixir 'elixir' erlang 'erlang' zig 'zig' swift 'swift' r 'r' clojure 'clojure' java 'openjdk' lisp 'sbcl' dart 'dart-sdk' ada '' julia 'julia' kotlin 'kotlin' ocaml 'ocaml' scala 'scala' moonbit '')
typeset -A AGENT_DEP=(forgecode '' tmuxai '' codex '' claude '' opencode '' pi 'node' omp '' jcode '' hermes 'node' crush '' grok '' reasonix '' droid '' goose '' aider '' copilot '' cline 'bun node' codebuddy '' kilo '' kimi 'node' qwen 'node' mcode 'bun node' qoder 'bun node' prime 'node uv' deepagents 'uv' openhands 'uv' deepcode 'bun node' atomic '' letta '' zerostack '' mimo '' opensci '')
...
QUICKLISP=$(( ${LANGS[(I)lisp]} > 0 )); ALIRE=$(( ${LANGS[(I)ada]} > 0 )); MOONBIT=$(( ${LANGS[(I)moonbit]} > 0 ))

for k in TUI LANGS AGENTS DEPS CORE_FORMULAE TUI_FORMULAE LANG_FORMULAE QUICKLISP ALIRE MOONBIT; print -r -- "$k='${(P)k}'"
