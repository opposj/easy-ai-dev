**Caveats:**
1. `warp-agent-cli`/`muse-code`/`fx`/`devin`/`kiro`/`kimchi`/`replit-agent` does not support BYOK in a natural way
...
- **Auth:** `KEY=<key>; jq -n '{deepseek:{type:"api_key",key:"'$KEY'"}}' > /root/.pi/agent/auth.json`
...
- **Auth:** `KEY=<key>; printf "DEEPSEEK_API_KEY=$KEY" > /root/.hermes/.env`
...
- **Auth:** `KEY=<key>; printf "DEEPSEEK_API_KEY=$KEY" > /root/.reasonix/.env`
...
- **Auth:** `KEY=<key>; jq -n '{customModels:[{model:"deepseek-v4-flash-vision-exp",displayName:"DeepSeek V4 Flash Vision Exp",baseUrl:"https://api.deepseek.com",apiKey:"'$KEY'",provider:"openai",maxOutputTokens:384000}]}' > /root/.factory/settings.json`
...
- **Auth:** `KEY=<key>; printf "api-key:\n- deepseek=$KEY" > /root/.aider.conf.yml`
...
- **Auth:** `KEY=<key>; jq -n '{deepseek:{type:"api_key",key:"'$KEY'"}}' > /root/.prime/agent/auth.json && jq -n '{defaultProvider:"deepseek",defaultModel:"deepseek-v4-flash-vision-exp",defaultThinkingLevel:"max",telemetry:{enabled:false}}' > /root/.prime/agent/settings.json`
...
- **Auth:** `KEY=<key>; printf "DEEPSEEK_API_KEY=$KEY" > /root/.deepagents/.env && printf '[models]\ndefault = "deepseek:deepseek-v4-flash-vision-exp"\n\n[models.providers.deepseek.params]\nreasoning_effort = "max"\n' > /root/.deepagents/config.toml`
...
- **Auth:** `KEY=<key>; jq -n '{llm:{model:"deepseek/deepseek-v4-flash-vision-exp",api_key:"'$KEY'",base_url:"https://api.deepseek.com"}}' > /root/.openhands/agent_settings.json`
...
- **Auth:** `KEY=<key>; jq -n '{env:{MODEL:"deepseek-v4-flash-vision-exp",BASE_URL:"https://api.deepseek.com",API_KEY:"'$KEY'"},thinkingEnabled:true,reasoningEffort:"max",telemetryEnabled:false}' > /root/.deepcode/settings.json`
